...
const ProductCard = ({ icon, title, description, features, isPrimary = false }: ProductCardProps) => {
  return (
    <Card className={`h-full flex flex-col ${isPrimary ? 'border-brand-500/50 shadow-lg shadow-brand-500/10' : ''}`}>
      <CardContent className="pt-6 flex-1 flex flex-col">
        ...
        <Button 
          ...
          className={`mt-6 w-full ${isPrimary ? 'bg-brand-500 hover:bg-brand-600' : ''}`}
        >
    ...
    <section id="products" className="py-16">
      <div className="container">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <ProductCard 
            icon={<Phone />}
            title="Call Catch Pro"
            description="AI receptionist and switchboard that never sleeps"
            features={[
              "24/7 call answering",
          ...
          <ProductCard 
            ...
            description="Targeted leads for home service professionals"
            features={[
